...
docker compose exec backup gzip -dc "$BACKUP_FILE_IN_CONTAINER" | docker compose exec -T db bash -c "PGPASSWORD=\"${DB_PASSWORD}\" pg_restore --clean --username \"${DB_USER}\" --dbname \"${DB_NAME}\" --host \"${DB_HOST}\" --port \"${DB_PORT}\""
