...
docker compose exec backup gzip -dc "$BACKUP_FILE_IN_CONTAINER" | docker compose exec -T --env "PGPASSWORD=$PGPASSWORD" db pg_restore --clean -U postgres --dbname "$DB_NAME" --host "$DB_HOST" --port "$DB_PORT"
